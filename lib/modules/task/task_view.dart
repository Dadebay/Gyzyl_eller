...
class _TaskViewState extends State<TaskView>
  ...
  Future<void> _clearNotificationForOpenedJob(int jobId) async {
    ...
    jobNotif.optimisticallyMarkCleared(jobId.toString());
    await jobNotif.clearByJob(jobId.toString());
  }
  ...
  Future<void> _refreshCurrentTabSilently() async {
    // Önce current tab refresh tetiklenir, ardından AllView
    // ve local "Taze" badge sayacı güncellenir.
    if (_tabController.index == 0) {
      controller.requestedRefreshController.requestRefresh();
      await controller.fetchRequestedJobs(isRefresh: true);
    } else {
      controller.processingRefreshController.requestRefresh();
      await controller.fetchProcessingJobs(isRefresh: true);
    }

    // UI hızlı güncellensin diye sayaçları yenile.
    if (Get.isRegistered<JobNotificationController>()) {
      await Get.find<JobNotificationController>().fetchNotificationCounters();
    }

    // AllView badge'leri için de data taze kalmalı.
    if (Get.isRegistered<AllController>()) {
      await Get.find<AllController>().fetchJobs(isRefresh: true);
    } else {
      await Get.put(AllController(), permanent: true)
              ...
              child: controller.processingJobs.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        // Each card gets its own Obx so counterResponse and
                        // _viewedMasterReplyJobIds changes trigger reactive
                        // rebuilds immediately when the user taps the card —
                        // same pattern as all_view.dart's jobs.refresh() +
                        // markViewed().
                        ...
                        return Obx(() {
                          final jobNotif =
                              Get.isRegistered<JobNotificationController>()
                                  ? Get.find<JobNotificationController>()
                                  : null;
                          // Count notifications for this specific job from
                          // the controller items (tab1: MASTER_REPLY + selected)
                          int notifCount = 0;
                          if (jobNotif != null) {
                            final items =
                                jobNotif.counterResponse.value?.items ?? [];
                            notifCount = items
                                .where((i) => i.jobId == job.id.toString())
                                .length;
                          }
                          return Stack(
                            clipBehavior: Clip.none,
                            children: [
                              Container(
                                ...
                                child: JobCard(
                                  ...
                                  notificationCountOverride: notifCount,
                                  onOpened: () =>
                                      _clearNotificationForOpenedJob(job.id),
                                  onDeleted: () =>
                                      controller.fetchProcessingJobs(
                                          isRefresh: true),
                                ),
                              ),
                            ],
                  ...
                  return Obx(() {
                    final jobNotif =
                        Get.isRegistered<JobNotificationController>()
                            ...
                            : null;
                    // Count notifications for this specific job from
                    // the controller items (tab0: REQUEST_SELECTED for other)
                    int notifCount = 0;
                    if (jobNotif != null) {
                      final items =
                          jobNotif.counterResponse.value?.items ?? [];
