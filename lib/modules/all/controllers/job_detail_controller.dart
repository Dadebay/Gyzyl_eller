...
class JobDetailController extends GetxController {
  ...
  Future<void> fetchJobDetail(int jobId) async {
    ...
    try {
      ...
      // Update the processingJobs list in TaskController so card reflects the latest state
      if (Get.isRegistered<TaskController>() && job.value != null) {
        try {
          final taskController = Get.find<TaskController>();
          // Update processingJobs list: find and replace the job with fresh data
          final existingIndex = taskController.processingJobs
              .indexWhere((j) => j.id == job.value!.id);
          if (existingIndex >= 0) {
            taskController.processingJobs[existingIndex] = job.value!;
            taskController.processingJobs.refresh();
            print(
                '🔄 [JobDetailController] Updated processingJobs[$existingIndex] with latest job data');
          }
        } catch (e) {
          print('⚠️ [JobDetailController] Failed to update processingJobs: $e');
        }
      }
  ...
  @override
  void onClose() {
    // Before disposing, fetch latest job data to ensure processingJobs has the
    // updated state (with new reviewId/rating if added) before showing card
    if (job.value != null && Get.isRegistered<TaskController>()) {
      fetchJobDetail(job.value!.id).catchError((e) {
        print('⚠️ [JobDetailController] onClose refresh failed: $e');
      });
    }
  ...
  Future<bool> replyToReview(String reviewId, String replyText) async {
    try {
      ...
      if (isSuccess) {
        ...
        if (Get.isRegistered<JobNotificationController>()) {
          print(
              '🔔 [replyToReview] reply başarılı → fetchNotificationCounters');
          Get.find<JobNotificationController>().fetchNotificationCounters();
        }
        return true;
      } else {
